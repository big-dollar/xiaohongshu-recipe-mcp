...
- 📁 **支持本地 HTML 文件解析**：现在不仅能直接传网页 URL，还能传入本地下载好的 `.html` 文件路径！遇到极强防爬的网站（如 Cloudflare 拦截）也能轻松应对！
- ⏳ **草稿箱超时驻留**：执行「暂存离开」后，自动将浏览器维持开启状态 300 秒（5分钟），留足时间让你当场点进草稿箱精调文案，到期后或关闭时自动优雅退出！
...
💡 **不仅支持网址，还直接支持传入你电脑里的本地 HTML 文件路径！**

### 1. 自动生成并存入草稿箱 (推荐)
- **工具名称**: `generate_and_save_draft_recipe`
- **能力描述**: 抓取图文并生成完美文案，然后**自动填入小红书，但不点击发布，而是点击“暂存草稿”！**（存草稿后浏览器会保留 5 分钟方便你立刻修改）
- **提示词示例**: 
  > *"帮我把这篇食谱抓下来，写好文案存到小红书草稿箱里等我修改：https://xxxx..."*
  > *"根据本地文件生成小红书美食笔记并暂存到草稿箱：D:/test/recipe.html"*
...
- **提示词示例**: 
  ...
  > *"解析桌面上这个网页源码文件并自动发布到小红书：C:/Users/xxx/Desktop/food.html"*
